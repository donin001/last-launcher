...
class AppListState extends ChangeNotifier {
  ...
  Set<String> get _appsInFolders => {
    for (final folder in _folders)
      for (final app in folder.apps)
        _compoundKey(app.packageName, app.isWorkApp),
  };
  ...
  void _pruneFolders() {
    ...
    for (var i = 0; i < _folders.length; i++) {
      ...
      final filteredApps = folder.apps
          .where(
            (a) => installedCompound.contains(
              _compoundKey(a.packageName, a.isWorkApp),
            ),
          )
          .toList();
  ...
  Future<void> addAppToFolder(String folderId, AppInfo app) async {
    ...
    if (folder.apps.any(
      (a) =>
          a.packageName == pinned.packageName &&
          a.isWorkApp == pinned.isWorkApp,
    )) {
      return;
    }
